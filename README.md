...
Algolia uses API Keys to verify users who call their API. Your API keys must be kept secure, so typically they’re only stored on your server. In order to take advantage of Algolia’s fast, globally-distributed search index, you want to allow users to call Algolia’s API directly from their browser, but you don’t want to give them complete access to your Admin API keys. Fortunately, there are ways around these limitations.

First, Algolia gives you search-only API keys, which gives users unrestricted access to search all indices of your application. This is okay for some applications, but if you don’t want every user to search every record, these keys are too broad. The next level of security they offer is a [secured API key](https://www.algolia.com/doc/guides/security/api-keys/#secured-api-keys-virtual), which is more restricted. You can generate a secured API key that limits access to a specific user, duration or index. That way, if a secured API key is compromised, it can't be used to gain unlimited access to your search data.

The bottleneck then becomes generation of your secured API key. Before searches can be made on Algolia, an API request would need to be sent to your server, where a secured API key would be requested from Algolia’s server. This extra call to your server can slow down a user’s first search and degrade their experience. One way to make that secured API key generation as fast as possible is to move it to the edge with a hosting platform like Fly.

 ![](images/secured_key_flowchart.jpg)

In this tutorial, you will see how to create an Algolia-hosted search application that generates secured API keys from Fly’s edge hosting platform. This makes every request - including a user’s first search - as fast as possible while maintaining your admin and search-only API keys’ integrity.
...
Create a new folder for your application. From this folder, install [Express](https://expressjs.com/) and the [Algolia search package](https://www.npmjs.com/package/algoliasearch).
...
Log into your Algolia account and go to your dashboard. From there, click the **API Keys** link on the left side-bar. Copy the **Application ID**, **Search-Only API Key**, and **Admin API Key**. You'll need these to test your Node app and to add as environment variables when you deploy it to Fly.
...
Now you are ready to start your Express app and load some data into Algolia. The code above creates an index and then loads the JSON data from `contacts.json` into Algolia. Copy the contents of [this `contacts.json` file](https://github.com/karllhughes/fly-algolia-secured-key/blob/main/contacts.json) which contains several hundred fake contacts:
...
You're now ready to start your Node app. Run the following from your terminal with your Algolia credentials:
...
Go to `localhost:8080/index` in your browser and you should see the message, "Index created" when the data import is complete. Now that you have data in your Algolia index, you are ready to create an endpoint to generate a secured key.

To confirm that the `Contact` index is up on Algolia, visit the Algolia Dashboard, on the left side bar click on **Indices**. You should see something like the following:

![Image of Indices](images/indices.png)

### Generating a Secured API Key
Now that you have data in your Algolia index, you're ready to create a Secured API Key to access it. The Secured API Key is important because it ensures that the unrestricted Search-Only Key provided by Algolia is not exposed. Malicious users can use the key to access unlimited search at the application's expense.
...
Since you already added the UI for this application in the `src/html/index.html` file above, you're now ready to test the application out. Restart the Node server using the same environment variables you did above and view the app running at `http://localhost:8080`.
...
Before you deploy your application to Fly, it needs to have a Dockerfile. Fly uses this to package up the app and ensure that all the dependencies are installed and code copied appropriately. Create a `Dockerfile` in the root of your application and add the following:
...
Next, sign up or sign into your Fly account:
...
You'll be asked for an application name. It's probably best to use the recommended name by fly, because it is always unique. Select your organization, the "Dockerfile" builder and the default port ("8080"). The `fly.toml` is created at the end of the session and should look something like this:
...
## Conclusion
In this tutorial, you've seen how to deploy a Node application to Fly and then use that application to generate a Secured API Key for Algolia. Your application then used that key to request results from Algolia's global search index without exposing your Search-Only API Key.
